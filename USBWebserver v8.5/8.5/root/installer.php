...
/* ==============================================================================================
	...
	$GLOBALS["SCRUB_LIST"]:	(empty)
		Use this list to scrub additional urls in the database.  This list be replace with 
		the 'Install URL' value on the main install page.  This is useful if you have urls 
		that need to be replaced. example:
		$GLOBALS["SCRUB_LIST"] = array("http://localhost/foo1/", "http://localhost/foo2/")
				
	$GLOBALS["SERIAL_STR_FIX"]: (true)
		Attempt to fix any serilized string data that was stored in the wordpress options table.
		This is required for plugins that store the full url as a value in the options table.
		
	$GLOBALS["SERIAL_TABLES"]: requires $GLOBALS["SERIAL_STR_FIX"] to be true
		By default the Duplicator only scrubs the wordpress options table.  If you would like other 
		tables to be scrubbed to fix serialized strings that are associted with the 'Install URL'
		and $GLOBALS["SCRUB_LIST"] urls then add the tables here.  The table will need to have
		a unique column index passed as the 'column_id' key and a column were the actual correction
		will be made in the 'column_value'.

================================================================================================= */

$GLOBALS["SCRUB_LIST"] 	   		= null; //array("");
$GLOBALS["SERIAL_STR_FIX"] 		= true;
$GLOBALS["MAX_TIME"] 			= 5000; //in seconds
...
ini_set("max_input_time",     "{$GLOBALS['MAX_TIME']}");
...
define('MSG_ERR_CONFIG', 			'<div class="error"><b style="color:#C16C1D;">INSTALL ERROR!</b><br/> A configuration file already exists in this location.  You have two options: <ul><li>Empty this root directory and recopy the package and installer.</li><li>Copy package from "' . DUPLICATOR_SSDIR_NAME . '" directory back up to your root (same location as installer.php) and delete the wp-config.php file.</li></ul></div>');
...
define('MSG_ERR_DBCLEANCHECK', 		'<div class="error"><b style="color:#C16C1D;">INSTALL ERROR!</b><br/> The database "%s" has %s tables.  The Duplicator only works with an EMPTY database.  Enable the "Allow Table Removal" checkbox to delete all tables and proceed with installation.  Please backup all your data before proceeding!<br/><br/>  Some hosting providers do not allow table removal from scripts like the Duplicator.  In this case you will need to login to your hosting providers control panel and remove the tables manually.  Please contact your hosting provider for further details. </div>');
define('MSG_ERR_ZIPNOTFOUND', 		'<div class="error"><b style="color:#C16C1D;">INSTALL ERROR!</b><br/> The packaged zip file was not found. Be sure the zip package is in the same directory as the installer file.  If you are trying to reinstall a package you can copy the package from the "' . DUPLICATOR_SSDIR_NAME . '" directory back up to your root (the same location as your installer.php file). </div>');
...
define('MSG_ERR_ZIPMANUAL', 		'<div class="error"><b style="color:#C16C1D;">INSTALL ERROR!</b><br/> When choosing manual package extraction, the contents of the package must already be extracted and the wp-config.php and database.sql files must be present in the same directory as the installer.php for the process to continue.  Please manually extract the package into the current directory before continuing in manual extraction mode.  </div>');
define('MSG_ERR_ZIPTOMANY', 		'<div class="error"><b style="color:#C16C1D;">INSTALL ERROR!</b><br/> When running the duplicator only one .zip archive file can be placed within the same directory as the installer.php file.  Please make sure the .zip is the correct Package you are trying to install and only one exists. </div>');
define('MSG_ERR_FWRITE', 			"<div class='error'><b style='color:#C16C1D;'>INSTALL ERROR!</b><br/> PHP on this web server currently does not have the proper access level to write to the document root which is currently <b>'{$GLOBALS['CURRENT_ROOT_PATH']}'</b>.  In order for the duplicator to proceed please enable PHP with the ability to write files.  If you're on a shared hosting environment please contact your hosting company and tell them you're getting warnings with writing files in PHP. </div>");
 ...
 */
function dinstaller_table_count($conn, $dbname) {
 ...
 */
function dinstaller_table_row_count($conn, $table_name) { 
 ...
 */
function dinstaller_log($msg, $loglevel = 1) {
...
/**
 *  SET_SERIALIZED_STRING
 *  Fixes the string length of a string object that has been serialized
 *  International strings will not return the currect length without setting: 'Enable MySQL character set' 
...
function dinstaller_set_serialized_string($sObject) {
	...
	$serial_string = preg_match( '/^s:[0-9]+:"(.*$)/s', trim($sObject), $match);
	//Nested serial string
	if ($serial_string) {
		$inner = preg_replace_callback($regex, 'dinstaller_serialize_callback', rtrim($match[1], '";')); 
		$serialized =  's:' . strlen(utf8_encode($inner)) . ':"' . $inner . '";' ;
	} else {
		$serialized = preg_replace_callback($regex, 'dinstaller_serialize_callback', $sObject); 
	}
	return $serialized;
} 

function dinstaller_serialize_callback($match) { 
     return 's:' . strlen(utf8_encode($match[2])); 
} 
...
if ($action == 'dbconnect-test') {
	$mysqli_conn = @mysqli_connect($dbhost, $dbuser, $dbpassword);
	...
	if (! $dbmake) {
		mysqli_select_db($mysqli_conn, $dbname) or die(sprintf(MSG_ERR_DBCREATE, $dbname));
	}
	
	if (! $dbclean) {
		$tblcount = dinstaller_table_count($mysqli_conn, $dbname);
		if ($tblcount > 0) {
...
<head>
	...
	<style type="text/css">
		...
		div#dbconn-test-border {border:10px solid #E4E4E4;}
		div#dbconn-test-hdr {font-weight:bold; background-color:#E4E4E4; padding:0px 5px 8px 0px;}
		div#dbconn-test-msg {background-color:#fff; padding:2px 15px; height:300px; overflow-y:scroll; border-radius:4px; }
		div#dbconn-test-close {background-color:#E4E4E4;font-size:14px;}
		
		div.connect {padding:18px; font-size:12px; border:1px solid silver; border-radius:5px; background-color:#fff; text-align:center; margin-top:15px; line-height:22px;background-color:#efefef;-webkit-box-shadow: 0 4px 2px -2px #777; -moz-box-shadow: 0 4px 2px -2px #777; box-shadow: 0 4px 2px -2px #777;}
		h3.detail-warning {color:grey; font-weight:bold}
	</style>
	<script type="text/javascript">
		//NOTE: We can't assume the user will always have an http connection, so we need to include jquery directly.
	...
	<script type="text/javascript">
		...
		editNewURL = function() {
			var msg =  'This is the URL that was generated when the package was created.  ';
				...
				msg += 'Only modify  this value if you know exactly what the package URL should be.  ';
				msg += 'See "General Settings" in the WordPress Administrator for more details.\n\n';
				msg += 'Are you sure you want to continue?';
			
			if (confirm(msg)) {
				$("#current_url").removeAttr('readonly');
				$("#current_url").removeClass('readonly');
				$('#edit_current_url').hide('slow');
			}
		}
		
		validateInstall = function () {
			$('#dbconn-test-msg').html("Attempting Connection.  Please wait...");
			$('#dbconn-test').css('display', 'block');
			var dbmake 	= $('input#dbmake').is(':checked')  ? 1 : 0;
			var dbclean = $('input#dbclean').is(':checked') ? 1 : 0;
			
			var q;
			q  = "dbhost=" 		+  $("input#dbhost").val() + "&";
			q += "dbname=" 		+  $("input#dbname").val() + "&";
			q += "dbuser=" 		+  $("input#dbuser").val() + "&";
			q += "dbpassword="  +  $("input#dbpassword").val() + "&";
			q += "dbmake="  	+  dbmake + "&";
			q += "dbclean="  	+  dbclean;
			$.ajax({
				type: "POST",
				url: window.location.href,
				data: q + "&action=dbconnect-test",
		...
		prepAdminPage = function() {
			$.ajax({type: "POST", url: "<?php echo "{$new_url}/wp-admin/" ?>", success: function(data){}	});
		}
		...
		$(document).ready(function() {
			getNewURL();
			acceptWarning();
			$("#form-installer").submit(function (event) {
				if ( $("#nurl").val() == "" ) 	{alert("The field intall url is required!"); return false;}
				if ( $("#dbhost").val() == "" ) {alert("The field database host is required!"); return false;}
				if ( $("#dbname").val() == "" ) {alert("The field database name is required!"); return false;}
				if ( $("#dbuser").val() == "" ) {alert("The field database user is required!"); return false;}
				...
				if (answer) {
					$("#installbtn").val('Processing Please Wait... ');
					return true;
				} else {
					return false;
				}
			...
			$('#hlp-link').click(function() {window.open('<?php echo DUPLICATOR_HELPLINK ?>')});
			$('a#dbconn-test-close-link').click(function() {$('#dbconn-test').hide(); });
		});
	</script>
</head>
<body>
...
<div id="content">
	<table cellspacing="0" class="content-title">
		<tr>
			<td></td>
			<td>&nbsp;&nbsp;</td>
			<td style="width:100%;"><h1>Duplicator - Installer</h1></td>
			<?php if ($action == null) : ?>
				<td style="white-space:nowrap;padding:4px"><a id="hlp-link" href="javascript:void(0)">Help</a>&nbsp;</td>
			<?php endif; ?>
		</tr>
	</table>
	...
	if ($action == 'process') {
		...
		//Link to Log
		echo "<div style='text-align:center; margin:10px;'><i>See <a href='installer-log.txt' target='_blank'>installer-log.txt</a> for an overview of this process.</i></div>";	
		
		$tryagain_html	 = "<div class='tryagain'><a href='javascript:window.history.back()' style='color:#444; font-weight:bold'>Try Again</a></div>";
		$log = "";
		...
		if (! $mysqli_conn) {
			die(MSG_FAIL_DBCONNECT . mysqli_connect_error() . $tryagain_html);
		}
			
		if (! $dbmake) {
			mysqli_select_db($mysqli_conn, $dbname) 
				or die(sprintf(MSG_ERR_DBCREATE, $dbname)  . $tryagain_html);
		} else {
			$sql = "CREATE DATABASE IF NOT EXISTS `{$dbname}`";
			mysqli_query($mysqli_conn, $sql);
			mysqli_select_db($mysqli_conn, $dbname) 
				or die(sprintf(MSG_FAIL_DBCONNECT_CREATE, $dbname) . $tryagain_html);
		}
			...
			if($result = mysqli_query($mysqli_conn, $sql)){
				...
				if (count($found_tables) > 0) {
					...
					foreach($found_tables as $table_name){
						...
						if(! $result = mysqli_query($mysqli_conn, $sql)){
							die(sprintf(MSG_FAIL_DBTRYCLEAN, $dbname) . $tryagain_html);
						} else {
							$log .=  "{$sql}\n";
						}
					}
				} else {
					$log = "No tables found to remove.";
				}
			} 
		}
		dinstaller_log("{$GLOBALS['SEPERATOR1']}");
		dinstaller_log('INSTALLER-LOG');
		dinstaller_log("{$GLOBALS['SEPERATOR1']}");
		dinstaller_log('START-PRECHECK:' . @date('h:i:s') );
		dinstaller_log("----------------------------------");
		dinstaller_log("ENVIROMENT:");
		dinstaller_log("version: {$GLOBALS['DUPLICATOR_INSTALLER_VERSION']}");
		dinstaller_log("log level: {$GLOBALS['LOG_LEVEL']}");
		dinstaller_log("php:" .  phpversion());
		dinstaller_log("php sapi: " .  php_sapi_name());
		...
		dinstaller_log("secure build name: 503dd375a2f245077_20120829_instantwordpress");
		dinstaller_log("----------------------------------");
		...
		//====================================================================================================
		dinstaller_log("{$GLOBALS['SEPERATOR1']}");
		dinstaller_log("PACKAGE-EXTRACTION");
		dinstaller_log("{$GLOBALS['SEPERATOR1']}");
		...
		dinstaller_log("{$GLOBALS['SEPERATOR1']}");
		dinstaller_log('START-FILES:' . @date('h:i:s') . "\n" );
		...
		if ($dbcharvalid) {
			dinstaller_log("ran 'Replace Invalid Characters' option\n" . $log);
		...
		if (count($GLOBALS["SCRUB_LIST"]) > 0) {
			for($i = 0; $i < count($GLOBALS["SCRUB_LIST"]); ++$i) {
				array_push($primary_list, $GLOBALS["SCRUB_LIST"][$i]);
			}
		}
		...
		//====================================================================================================
		dinstaller_log("{$GLOBALS['SEPERATOR1']}");
		dinstaller_log('DATABASE-ROUTINES');
		dinstaller_log("{$GLOBALS['SEPERATOR1']}");
		dinstaller_log('START-DB:' . @date('h:i:s') . "\n" );
		dinstaller_log("db create: "  . var_export($dbmake, true));
		dinstaller_log("db clean: "   . var_export($dbclean, true));
		dinstaller_log("db replace characters: ". var_export($dbcharvalid, true));
		dinstaller_log("db mysqli charset: "    . var_export($dbmysqlichar, true));
		dinstaller_log("db wait_timeout: {$GLOBALS['MAX_TIME']}");
		
		mysqli_query($mysqli_conn, "SET wait_timeout = {$GLOBALS['MAX_TIME']}");
		...
		dinstaller_log("----------------------------------");
		dinstaller_log("DATABASE RESULTS");
		dinstaller_log("processed queries: {$temp} ");
		dinstaller_log("process time: " . DInstaller::elapsed_time($profile_end, $profile_start) . "\n");
		
		$table_count = 0;
		dinstaller_log("Table Record Counts");
		if ($result = mysqli_query($mysqli_conn, "SHOW TABLES")) {
			while ($row = mysqli_fetch_array($result, MYSQLI_NUM)) {
				$table_rows = dinstaller_table_row_count($mysqli_conn, $row[0]);
				dinstaller_log("{$row[0]}:{$table_rows}");
				$table_count++;
			}
		...
		if ($GLOBALS["SERIAL_STR_FIX"]) {
			dinstaller_log("----------------------------------");
			dinstaller_log("START SERIALIZATION:" . @date('h:i:s') . "\n");
			$fix_count	= 0;
			$seperator1  = str_repeat("-----", 15);
			
			foreach($GLOBALS["SERIAL_TABLES"] as $table_name => $column_info) {
				
				$table 	 = $table_name; 
				$col_id  = $column_info['column_id'];
				$col_val = $column_info['column_value'];
				$sql 	 = "SELECT {$col_id}, {$col_val} FROM {$table}" ;
				$retid 	 = mysqli_query($mysqli_conn, $sql);
				$serial_count = @mysqli_num_rows($retid);
			
				dinstaller_log("serialization query ({$serial_count}):\n{$sql} \n");
				while ($row = @mysqli_fetch_array($retid)) {
				
					$base64coverted = false;
					if (base64_decode($row[$col_val], true))
					{
						$decoded = base64_decode($row[$col_val], true);
						if (dinstaller_is_serialized($decoded)) {
							for($i = 0; $i < count($primary_list); ++$i) {
								$old_url 		= $primary_list[$i];
								$row[$col_val]	= str_replace($old_url, $new_url, $decoded);
								$base64coverted = true;
							}
						}
					} 
					...
					if (strpos($row[$col_val], 's:') !== false) {
						...
						if ($old_val != $new_val) {
							...
							$sql    = "UPDATE {$table} SET {$col_val} = '" . mysqli_real_escape_string($mysqli_conn, $new_val) . "' WHERE {$col_id} = '{$index}' ";
			...
			dinstaller_log("END SERIALIZATION:" . @date('h:i:s') . "\n");
			$log = '';
		}
		...
		//====================================================================================================
		dinstaller_log("{$GLOBALS['SEPERATOR1']}");
		dinstaller_log('START FINAL CLEANUP');
		dinstaller_log("{$GLOBALS['SEPERATOR1']}");
		dinstaller_log('START-CLEANUP:' . @date('h:i:s') . "\n" );
		...
		$currdata = parse_url("http://127.0.0.1:4001/wordpress"); 
...
HTACCESS;
		...
		$html .= "<div style='margin-top:4px'><i style='font-size:11px; color:#999'>installer version: {$GLOBALS['DUPLICATOR_INSTALLER_VERSION']} </i></div>";
		...
		dinstaller_log('END FINAL:' . @date('h:i:s') . "\n");
		fclose($GLOBALS["LOG_FILE_HANDLE"]);
		dinstaller_log("PROCESS CAME TO FULL COMPLETION \n");
	}
	?>
	...
	<?php if  ($action == null) : ?>
		...
		<form id="form-installer" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" >
			...
			<table>
				<tr>
					<td style="width:100%"><b>Settings</b></td>
					<td>
			...
			</table><hr size="1" style="margin-top:0px" />
			...
			<table width="100%" border="0" cellspacing="2" cellpadding="2" class="table-inputs">
				...
				<tr valign="top">
					...
					<td>
						<input type="text" name="current_url" id="current_url" value="http://127.0.0.1:4001/wordpress" readonly="true"  class="readonly" />
				...
				<tr>
					<td>Site Title</td>
					<td><input type="text" name="site_title" id="site_title" value="Instant WordPress" /></td>
				</tr>
			</table><br/>
			...
			<div class="warning-area">
				<input id="accept-warnings" type="checkbox" onclick="acceptWarning()" /> <label for="accept-warnings">I have read all warnings &amp; notices</label><br/>
			</div>
		
			
			<div class="div-buttons">
				<input type="button" id="reloadbtn" value="  Reload  " class="button" onclick="window.location.reload()"/>
				<input type="submit" id="installbtn" value="  Install  "  name="submit" class="button" disabled="true"/>
			</div>
			
		</form>
		<div style='margin-top:-7px'><i style='font-size:11px; color:#999'>installer version: <?php echo $GLOBALS['DUPLICATOR_INSTALLER_VERSION']?> </i></div>
	<?php endif; ?> <br/>
