...
function headway_register_visual_editor_box($class) {
	...
	add_action('headway_visual_editor_init', create_function('', 'return headway_register_visual_editor_box_callback(\'' . $class . '\');'));
...
abstract class HeadwayVisualEditorBoxAPI {
	...
	public function build_box() {
		...
		$attributes = array(
			...
			'load_with_ajax_callback' => htmlspecialchars('(function(args){' . $this->load_with_ajax_callback . '})')
		);
		...
		echo '<div class="box-content-bg"><div class="box-content">';
			
			if ( !$this->load_with_ajax )
				$this->content();
			
		echo '</div></div></div><!-- div#box-' . $this->id . ' -->';
